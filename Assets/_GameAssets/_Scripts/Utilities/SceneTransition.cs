...
public static class SceneTransition
{
    ...
    private static async Awaitable TransitionAsync(string targetSceneName, FadeConfig fadeOutConfig, SO_ScreenFadeRef fadeRef, SO_TransformRef xrOriginRef = null)
    {
        ...
        try
        {
            ...
            // Wait one frame so Start() runs (resets alpha=0), then set opaque after.
            ScreenFade loadingFade = null;
            CacheLoadingScreenRefs(loadingIndex, ref loadingFade);

            OnProgress?.Invoke(0f);

            // Wait for loading screen's Start() to run, then lock it opaque so it covers the scene unload.
        ...
        {
            Debug.LogError($"[SceneTransition] Exception during transition to '{targetSceneName}': {e}");
        }
        finally
        {
