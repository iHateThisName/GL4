...
public class Global2DSound : MonoBehaviour {
    ...
    [SerializeField] private string paramterWindName = "Wind State";
    ...
    private void HandleLocationTypeChanged(PlayerTemperatureSimulator.EnumLocationType type) {
        ...
        if (type == PlayerTemperatureSimulator.EnumLocationType.Normal || type == PlayerTemperatureSimulator.EnumLocationType.Warm) {
            this.emitter.SetParameter(this.paramterWindName, (int)EnumWindState.Indoor);
        } else if (type == PlayerTemperatureSimulator.EnumLocationType.Cold || type == PlayerTemperatureSimulator.EnumLocationType.Shack) {
            this.emitter.SetParameter(this.paramterWindName, (int)EnumWindState.Outdoor);
        }
    }
    [ContextMenu("Movment Idle")] public void SetMovementIdle() => this.emitter.SetParameter(this.paramterMovementName, (int)EnumMovementState.Idle);
    [ContextMenu("Movment Walking")] public void SetMovementWalking() => this.emitter.SetParameter(this.paramterMovementName, (int)EnumMovementState.Walking);
    [ContextMenu("Movment Running")] public void SetMovementRunning() => this.emitter.SetParameter(this.paramterMovementName, (int)EnumMovementState.Running);
}
